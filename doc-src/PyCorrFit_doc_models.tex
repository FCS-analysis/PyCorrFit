...
\label{sec:imple}
This is an overview of all the model functions that are currently\footnote{\today} implemented in \textit{PyCorrFit}. To each model a unique model ID is assigned by PyCorrFit. Most of the following information is also accessible from within \textit{PyCorrFit} using the \textbf{Page info} tool.
...
\subsection{TIR-FCS with Gaussian-shaped lateral detection volume}
\label{sec:imple.tirfcsg}
...
\subsection{TIR-FCS with a square-shaped lateral detection volume}
\label{sec:imple.tirfcss}
