    ...
    def processData(self):
        ...
        if self.ext == 'csv':
            ...
            if self.subChanArr == None:
                ...
                self.exit = True
                self.par_obj.image_status_text.showMessage(
                    "Your sample is not in the correct format.")
        ...
        self.photonDecayCh1, self.decayScale1 = delayTime2bin(np.array(
            self.dTimeArr), np.array(self.subChanArr), self.ch_present[0], self.winInt)

        if self.numOfCH == 2:
            self.photonDecayCh2, self.decayScale2 = delayTime2bin(np.array(
                self.dTimeArr), np.array(self.subChanArr), self.ch_present[1], self.winInt)

        # Time series of photon counts. For visualisation.
        self.timeSeries1, self.timeSeriesScale1 = delayTime2bin(np.array(
            self.trueTimeArr)/1000000, np.array(self.subChanArr), self.ch_present[0], self.photonCountBin)
        ...
        raw_count = np.average(self.timeSeries1)
        var_count = np.var(self.timeSeries1)

        self.brightnessNandBCH0 = (
            ((var_count - raw_count)/(raw_count))/(float(unit)))
        ...
        if self.numOfCH == 2:
            ...
            self.timeSeries2, self.timeSeriesScale2 = delayTime2bin(np.array(
                self.trueTimeArr)/1000000, np.array(self.subChanArr), self.ch_present[1], self.photonCountBin)
            unit = self.timeSeriesScale2[-1]/self.timeSeriesScale2.__len__()
            self.kcount_CH2 = np.average(self.timeSeries2)
            # This is the unnormalised intensity count for int_time duration (the first moment)
            raw_count = np.average(self.timeSeries2)
        ...
        if self.fit_obj != None:
            ...
            if self.numOfCH == 2:
                ...
                self.objId3.autoNorm = np.array(
                    self.autoNorm[:, 1, 1]).reshape(-1)
                ...
                self.objId2.autoNorm = np.array(
                    self.autoNorm[:, 0, 1]).reshape(-1)
                ...
                self.objId4.autoNorm = np.array(
                    self.autoNorm[:, 1, 0]).reshape(-1)
...
class subPicoObject():
    def __init__(self, parentId, xmin, xmax, TGid, par_obj):
        ...
        self.nameAndExt = os.path.basename(self.filepath).split('.')
        self.name = 'TG-'+str(self.unqID)+'-xmin_'+str(round(xmin, 0)) + \
            '-xmax_'+str(round(xmax, 0))+'-'+self.nameAndExt[0]
    ...
    def fitToParameters(self):
        ...
        for art in self.param:
            ...
            if self.param[art]['to_show'] == True:
                ...
                param.add(art, value=float(self.param[art]['value']), min=float(
                    self.param[art]['minv']), max=float(self.param[art]['maxv']), vary=self.param[art]['vary'])
        ...
        # Run the fitting.
        res = minimize(self.residual, param, args=(
            scale[self.indx_L:self.indx_R+1], data[self.indx_L:self.indx_R+1], self.parentFn.def_options))
        ...
        # self.parentFn.updateTableFirst();
        self.model_autoNorm = equation_(
            param, scale[self.indx_L:self.indx_R+1], self.parentFn.def_options)
    ...
    def load_from_file(self, channel):
        ...
        if self.ext == 'fcs':
            ...
            while read == True:
                ...
                corrObj.autoNorm = np.array(
                    tdata).astype(np.float64).reshape(-1)
                corrObj.autotime = np.array(tscale).astype(
        ...
        if self.ext == 'SIN':
            ...
            self.autotime = np.array(tscale).astype(
                np.float64).reshape(-1)*1000
        ...
        if self.ext == 'csv' or self.ext == 'CSV':
            ...
            if version >= 2:
                ...
                if numOfCH == 1:
                    ...
                    self.autoNorm = np.array(tdata).astype(
                ...
                if numOfCH == 2:
                    ...
                    self.autotime = np.array(tscale).astype(
                        np.float64).reshape(-1)
                    corrObj2.autotime = np.array(
                        tscale).astype(np.float64).reshape(-1)
                    corrObj3.autotime = np.array(
                        tscale).astype(np.float64).reshape(-1)

                    self.autoNorm = np.array(tdata0).astype(
                        np.float64).reshape(-1)
                    corrObj2.autoNorm = np.array(
                        tdata1).astype(np.float64).reshape(-1)
                    corrObj3.autoNorm = np.array(
                        tdata2).astype(np.float64).reshape(-1)
